...
#        self.initialize_weights()
    def forward(self, img):
        group_size = img.size(0)
        fea = self.prnet(img)           #### layer4=(B*N, C, H, W)  
        out4 = unsqz_fea(fea[3], group_size)        #### fea5=(B,N,392,H,W)
        out3 = unsqz_fea(fea[2], group_size)        #### fea5=(B,N,192,H,W)
        out2 = unsqz_fea(fea[1], group_size)
        ...
        spa_masks = spatial_optimize(gc4_out, group_size).cuda()
...
def spatial_optimize(fmap, group_size):
    ...
    for i in range(len(fmap_split)):
        ...
        with torch.enable_grad():
            spatial_s = nn.Parameter(torch.sqrt(49 * group_size * torch.ones((49 * group_size, 1))) / (49 * group_size), requires_grad=True)
...
#                   print('iter: [%.4f], loss: [%.4f], dloss:[%.4f], floss: [%.4f]' %(iter, all_loss, d_loss, f_spa_loss))
        ...
        result_map = spatial_s.data.view(group_size, 1, 7, 7)
...
def row_normalize(mx):
    ...
    return mx
def unsqz_fea(dim4_data, group_size):
    split_data = torch.split(dim4_data, group_size, dim=0)
